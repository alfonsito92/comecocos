 ...
 * @author cristian
 */
public class mueveNaranja implements Runnable {
    private int delay;
     ...
     * la hebra. 
     */
    public mueveNaranja(CocosFrame fr,int nivel){
        frame=fr;
        lvl=nivel;
    ...
    public void run(){
           ...
           Random aleatorio =new Random();
            try {
                ...
                Thread.sleep(delay);
                frame.getNaranja().setChoque(true);
                Pac p=frame.getPac();
                
                int contador=0;
                
                if(contador2<3){
                    frame.getRejilla().mueveFantasma(3,frame.getNaranja(),p);
                    contador2++;
                }else if(contador2<6){
                    frame.getRejilla().mueveFantasma(1,frame.getNaranja(),p);
                    contador2++;
                } else if(frame.getRejilla().getEstado()){{
                
                while(frame.getNaranja().getChoque()&&contador<10){
                if(frame.getPac().getXOrigen()-frame.getNaranja().getXOrigen()<=0){
                    if(frame.getPac().getYOrigen()-frame.getNaranja().getYOrigen()<=0)
                        frame.getRejilla().mueveFantasma(aleatorio.nextInt(2)*3,frame.getNaranja(),p);
                else if(frame.getPac().getYOrigen()-frame.getNaranja().getYOrigen()>=0)
                        frame.getRejilla().mueveFantasma(aleatorio.nextInt(2)*2,frame.getNaranja(),p);
                }else{ if (frame.getPac().getYOrigen()-frame.getNaranja().getYOrigen()<=0)
                        frame.getRejilla().mueveFantasma(aleatorio.nextInt(2)*2+1,frame.getNaranja(),p);
                else if(frame.getPac().getYOrigen()-frame.getNaranja().getYOrigen()>=0)
                        frame.getRejilla().mueveFantasma(aleatorio.nextInt(2)+1,frame.getNaranja(),p);
                }
