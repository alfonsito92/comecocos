 ...
 * @author cristian
 */
public class mueveRojo implements Runnable {
 private int delay;
     ...
     * la hebra. 
     */
    public mueveRojo(CocosFrame fr,int nivel){
        frame=fr;
        lvl=nivel;
    ...
    public void run(){
           ...
           Random aleatorio =new Random();
            try {
                ...
                Thread.sleep(delay);
                frame.getRojo().setChoque(true);
                Pac p=frame.getPac();
                
                int contador=0;
                
                if(contador2<3){
                    frame.getRejilla().mueveFantasma(3,frame.getRojo(),p);
                    contador2++;
                }else if(contador2<6){
                    frame.getRejilla().mueveFantasma(0,frame.getRojo(),p);
                    contador2++;
                } else if(frame.getRejilla().getEstado()){{
                
                while(frame.getRojo().getChoque()&&contador<10){
                if(frame.getPac().getXOrigen()-frame.getRojo().getXOrigen()<=0){
                    if(frame.getPac().getYOrigen()-frame.getRojo().getYOrigen()<=0)
                        frame.getRejilla().mueveFantasma(aleatorio.nextInt(2)*3,frame.getRojo(),p);
                else if(frame.getPac().getYOrigen()-frame.getRojo().getYOrigen()>=0)
                        frame.getRejilla().mueveFantasma(aleatorio.nextInt(2)*2,frame.getRojo(),p);
                }else{ if (frame.getPac().getYOrigen()-frame.getRojo().getYOrigen()<=0)
                        frame.getRejilla().mueveFantasma(aleatorio.nextInt(2)*2+1,frame.getRojo(),p);
                else if(frame.getPac().getYOrigen()-frame.getRojo().getYOrigen()>=0)
                        frame.getRejilla().mueveFantasma(aleatorio.nextInt(2)+1,frame.getRojo(),p);
                }
